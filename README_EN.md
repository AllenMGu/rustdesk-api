...
8. The address-book view can select personal, owned, or authorized shared books and filter peers by that book's tags. The API applies the existing read-permission rules before returning these profiles.
9. A dedicated General, Network, Display, Account, and About settings page persists supported V1 defaults for theme, language, servers, display mode, image quality, remote cursor, audio, and clipboard. V1 remains explicitly VP9-only rather than presenting non-functional VP8/AV1 choices.
8. V2-style peer navigation provides Recent, Favorites, Address Book, registered Devices, search, multi-select, and grid/list views. Because browsers cannot use the desktop client's UDP LAN discovery, Devices comes from the authenticated API account and heartbeat state instead of pretending to perform LAN scanning.
