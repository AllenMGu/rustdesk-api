...
class GitHubArtifactPollingTests(TestCase):
    ...
    def test_defines_platform_specific_output_sets(self):
        ...
        self.assertEqual(
            {
                "CutiaRustDesk-aarch64.apk",
                "CutiaRustDesk-armv7.apk",
                "CutiaRustDesk-x86_64.apk",
            },
            _required_output_names(github_run),
        )
        self.assertTrue(
            _valid_output_for_run("CutiaRustDesk-aarch64.apk", github_run)
        )
        self.assertFalse(
    ...
    def test_combines_multiple_android_artifacts(
        ...
        for index, arch in enumerate(("aarch64", "armv7", "x86_64"), start=1):
            archive = io.BytesIO()
            with zipfile.ZipFile(archive, "w") as output:
                output.writestr(f"CutiaRustDesk-{arch}.apk", arch.encode())
            response = Mock()
            response.iter_content.return_value = [archive.getvalue()]
            response.raise_for_status.return_value = None
        ...
        with TemporaryDirectory() as artifact_root, override_settings(
            ...
            for arch in ("aarch64", "armv7", "x86_64"):
                self.assertEqual(
                    arch.encode(),
                    (destination / f"CutiaRustDesk-{arch}.apk").read_bytes(),
                )
